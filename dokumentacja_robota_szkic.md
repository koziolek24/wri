...
_Rys. 1. Plansza testowa z czarną linią, kolorowymi kaflami oraz robotem wykonującym przejazd._
...
| Czujnik koloru EV3                |     2 | Wykrywanie linii, kolorowych zjazdów i kolorowych kafli |
...
| `COLOR_GREEN`   | `Green` | Zielony kolor zjazdu i kafla           |
| `COLOR_RED`     | `Red`   | Czerwony kolor zjazdu i kafla          |
...
- stabilność odczytów na czerwonym i zielonym kaflu,
...
5. Robot dojeżdża do zielonego kafla.
...
flowchart TD
    ...
    PickupTile{Oba czujniki widzą kafel pobrania?}
    ...
    DropoffTile{Oba czujniki widzą kafel odstawienia?}
...
W stanie dojazdu do kolorowego kafla program dopuszcza śledzenie więcej niż jednego koloru. Dla dojazdu do pobrania robot może traktować jako linię zarówno `Green`, jak i `Black`. Dla dojazdu do odstawienia robot może traktować jako linię zarówno `Red`, jak i `Black`. Dzięki temu robot może przejechać fragment czarnej trasy między kolorowym rozjazdem a kolorowym kaflem.

### 5.6. Procedura pobrania obiektu

Procedura pobrania jest wykonywana po dojechaniu na zielony kafel, czyli gdy oba czujniki widzą kolor `Green`.
...
Procedura odstawienia jest wykonywana po dojechaniu na czerwony kafel, czyli gdy oba czujniki widzą kolor `Red`.

Kolejne kroki procedury:

1. Robot zatrzymuje napęd.
2. Chwytak zostaje opuszczony przez silnik średni.
3. Obiekt zostaje odstawiony na kaflu docelowym.
...
| Wjazd na zielony kafel     | Sprawdzenie dojazdu do punktu pobrania        | Zaliczone                          |
| Pobranie obiektu           | Sprawdzenie działania chwytaka                | Zaliczone                          |
| Obrót o około 180 stopni   | Sprawdzenie powrotu z kafla pobrania          | Zaliczone po dobraniu czasu obrotu |
...
| Wjazd na czerwony kafel    | Sprawdzenie dojazdu do punktu docelowego      | Zaliczone                          |
...
|     2 | Transporter  | Green -> Red         | Zaliczone | Robot pobrał obiekt z zielonego kafla i odstawił go na czerwonym kaflu |
...
Podczas testów zauważono, że odczyty z czujników mogą zależeć od oświetlenia oraz dokładnego miejsca na kaflu. Najbardziej problematyczne były granice między czarną linią, białym tłem i kolorowymi elementami planszy.

W praktyce konieczne było wielokrotne testowanie tych samych manewrów:

- wjazdu w zielony zjazd,
- dojazdu do zielonego kafla,
...
Najlepiej sprawdziły się proste rozwiązania oparte na dwóch czujnikach koloru i maszynie stanów. Dzięki podziałowi programu na stany łatwo było oddzielić jazdę po linii, dojazd do kafla pobrania, procedurę pobrania, dojazd do kafla odstawienia i zakończenie zadania.
